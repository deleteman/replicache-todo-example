...
export const mutators = {
  ...
  isStarted: async(tx: ReadTransaction, id: string) => {
    const todo: Todo = await tx.get(id) as Todo;
    return todo.startedOn != null && (typeof todo.startedOn != "undefined")
  },

  getWorkingWindows: async(tx: ReadTransaction, id: string) => {
    const todo: Todo = await tx.get(id) as Todo;
    return todo.workingWindows || []
  },

  getCurrentWindowStart: async(tx: ReadTransaction, id: string) => {
    const todo: Todo = await tx.get(id) as Todo;
    return todo.currentWorkingOn || 0;
  },
